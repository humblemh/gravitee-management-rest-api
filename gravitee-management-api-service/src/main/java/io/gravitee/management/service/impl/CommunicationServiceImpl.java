...
import static io.gravitee.management.service.impl.CommunicationServiceImpl.MesssageEvent.MESSAGE_SENT;
...
public class CommunicationServiceImpl extends AbstractService implements CommunicationService {
    ...
    @Override
    public int create(String apiId, CommunicationEntity message) {
    ...
    @Override
    public int create(CommunicationEntity message) {
        assertMessageNotEmpty(message);
    ...
    private int send(Api api, CommunicationEntity message, Set<String> recipientsId) {
    ...
    @Override
    public Set<String> getRecipientsId(CommunicationEntity message) {
        if (CommunicationChannel.HTTP.equals(message.getChannel())) {
    ...
    @Override
    public Set<String> getRecipientsId(Api api, CommunicationEntity message) {
        if (message != null && CommunicationChannel.HTTP.equals(message.getChannel())) {
            return Collections.singleton(message.getRecipient().getUrl());
        }
        assertRecipientsNotEmpty(message);
        CommunicationRecipientEntity recipientEntity = message.getRecipient();
    ...
    private void assertMessageNotEmpty(CommunicationEntity communicationEntity) {
        if (    communicationEntity == null ||
                (StringUtils.isEmpty(communicationEntity.getTitle()) && StringUtils.isEmpty(communicationEntity.getText()))) {
            throw new MessageEmptyException();
        }
    }

    private void assertRecipientsNotEmpty(CommunicationEntity communicationEntity) {
        if (    communicationEntity == null ||
                communicationEntity.getRecipient() == null ||
                communicationEntity.getChannel() == null ||
                communicationEntity.getRecipient().getRoleScope() == null ||
                communicationEntity.getRecipient().getRoleValues() == null ||
                communicationEntity.getRecipient().getRoleValues().isEmpty()) {
            throw new MessageRecipientFormatException();
        }
    }

    private Map<String, Object> getPortalParams(Api api, CommunicationEntity message) {
    ...
    private String getPostMessage(Api api, CommunicationEntity message) {
